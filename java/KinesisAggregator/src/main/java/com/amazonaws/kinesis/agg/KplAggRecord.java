 ...
 */
@NotThreadSafe
public class KplAggRecord
{
    // Serialization protocol constants via the KPL Protobuf specification at
 ...
 // Kinesis Limits (https://docs.aws.amazon.com/kinesis/latest/APIReference/API_PutRecord.html)
    private static final int MAX_BYTES_PER_RECORD = 1048576; // 1 MB (1024 * 1024)
    ...
    public byte[] toRecordBytes()
    {
        ...
        // The way Java's API works is that write(byte[]) throws IOException on a ByteArrayOutputStream, but 
        // write(byte[],int,int) doesn't so that's why we're using the long version of "write" here
    ...
    public boolean addUserRecord(String partitionKey, String explicitHashKey, byte[] data)
    {
        ...
        Record.Builder newRecord = Record.newBuilder().setData(data != null ? ByteString.copyFrom(data) : ByteString.EMPTY);
     ...
     * PutRecordRequest. This method has no side effects (i.e. it will not
     * clear the current contents of the aggregated record).
     * 
     * @param streamName The Kinesis stream name where this PutRecordRequest will be sent.
     ...
     * clear the current contents of the aggregated record).
     * 
     * @param streamName The Kinesis stream name where this PutRecordsRequestEntry will be sent.
    ...
    {
        final int maxAllowableDataLength = MAX_BYTES_PER_RECORD - KPL_AGGREGATED_RECORD_MAGIC.length - this.md5.getDigestLength();
     ...
     * 
     * @param partitionKey The string containing the input partition key to validate.
     */
    private void validatePartitionKey(final String partitionKey)
    {
        ...
        {
            throw new IllegalArgumentException("Invalid parition key. Length must be at least " + PARTITION_KEY_MIN_LENGTH + " and at most "
     ...
     * 
     * @param explicitHashKey The string containing the input explicit hash key to validate.
     */
        ...
        try
        {
            ...
            if (b.compareTo(UINT_128_MAX) > 0 || b.compareTo(BigInteger.ZERO) < 0)
            {
                throw new IllegalArgumentException(
                        "Invalid explicitHashKey, must be greater or equal to zero and less than or equal to (2^128 - 1), got " + explicitHashKey);
            }
        }
