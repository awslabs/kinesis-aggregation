...
public class SampleAggregatorProducer {
	 ...
	 * stream name.
	 */
	private static void sendRecord(AmazonKinesis producer, String streamName,
			KinesisAggRecord aggRecord) {
		if (aggRecord == null || aggRecord.getNumUserRecords() == 0) {
			return;
		}

		System.out.println("Submitting record EHK="
				+ aggRecord.getExplicitHashKey() + " DataSize="
				+ aggRecord.getSizeBytes());
		producer.putRecord(aggRecord.toPutRecordRequest(streamName));
		System.out.println("Completed record EHK="
				+ aggRecord.getExplicitHashKey());
	}
	 ...
	 * records to Kinesis.
	 */
	private static void sendViaCallback(AmazonKinesis producer,
			String streamName, KplAggregator aggregator) {
	 ...
	 * Kinesis.
	 */
	private static void sendViaBatch(AmazonKinesis producer, String streamName,
			KplAggregator aggregator) {
		System.out.println("Creating " + ProducerConfig.RECORDS_TO_TRANSMIT
				+ " records...");
		for (int i = 1; i <= ProducerConfig.RECORDS_TO_TRANSMIT; i++) {
			byte[] data = ProducerUtils.randomData(i,
	...
	public static void main(String[] args) {
		...
		final AmazonKinesis producer = ProducerUtils
				.getKinesisProducer(regionName);
