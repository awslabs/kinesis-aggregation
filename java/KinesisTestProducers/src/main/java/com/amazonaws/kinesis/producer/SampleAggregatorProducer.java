...
import com.amazonaws.kinesis.agg.KplAggRecord;
import com.amazonaws.kinesis.agg.KplAggregator;
import com.amazonaws.services.kinesis.AmazonKinesis;

/**
 * A sample of how to use the KplAggregator to transmit records to Kinesis.
 */
public class SampleAggregatorProducer
{
     ...
     * stream name.
     */
    private static void sendRecord(AmazonKinesis producer, String streamName, KplAggRecord aggRecord)
    {
        ...
        System.out.println("Submitting record EHK=" + aggRecord.getExplicitHashKey() 
                                                    + " NumRecords=" + aggRecord.getNumUserRecords()
                                                    + " NumBytes=" + aggRecord.getSizeBytes());
        try
        {
    ...
    /**
     * Flush out and send any remaining records from the aggregator and then wait for all pending transmissions
     * to finish.
     */
    private static void flushAndFinish(AmazonKinesis producer, String streamName, KplAggregator aggregator)
    {
        //Do one final flush & send to get any remaining records that haven't triggered a callback yet
        KplAggRecord finalRecord = aggregator.clearAndGet();
    ...
    {
        //add a lambda callback to be called when a full record is ready to transmit
    ...
    private static void sendViaBatch(AmazonKinesis producer, String streamName, KplAggregator aggregator)
    {
        ...
        for (int i = 1; i <= ProducerConfig.RECORDS_TO_TRANSMIT; i++)
        {
            ...
            //addUserRecord returns non-null when a full record is ready to transmit
