...
function getSizeIncrement(value) {
	...
	if (value) {
		...
		newBytes += calculateVarIntSize(value.length); // size of value length value
	...
	try {
		...
		records
				.map(function(record) {
					...
					// add the AggregatedRecord object with partition and hash key indexes
					putRecords
							.push({
		...
		if (debug) {
			...
			console.log("Size Bytes = " + f.length);
		}
	...
	records
			.map(function(record) {
				...
				if(record.ExplicitHashKey) {
					...
					if(self.explicitHashKeyTable.hasOwnProperty(record.ExplicitHashKey))
					{
				...
				if(typeof(record.Data) === 'string') {
					record.Data = new Buffer(record.Data,'utf8');
				}
